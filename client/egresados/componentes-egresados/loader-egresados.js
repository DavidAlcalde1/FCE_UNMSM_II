...
async function cargarPlantilla() {
  try {
    // 1. Cargar header
    const headHTML = await fetch('egresados/componentes-egresados/header-egresados.html').then(r => r.text());
    const headerDiv = document.getElementById('header-egresados-include');
    ...
    // 3. Cargar footer
    const footHTML = await fetch('egresados/componentes-egresados/footer-egresados.html').then(r => r.text());
    const footerDiv = document.getElementById('footer-egresados-include');
