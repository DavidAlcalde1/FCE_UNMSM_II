...
async function cargarPlantilla() {
  try {
    // 1. Cargar header
    const headHTML = await fetch('./bienestar/componentes-bienestar/header-bienestar.html').then(r => r.text());
    const headerDiv = document.getElementById('header-bienestar-include');
    ...
    // 3. Cargar footer
    const footHTML = await fetch('./bienestar/componentes-bienestar/footer-bienestar.html').then(r => r.text());
    const footerDiv = document.getElementById('footer-bienestar-include');
