...
async function cargarPlantilla() {
  try {
    // 1. Cargar header
    const headHTML = await fetch('./biblioteca/componentes-biblioteca/header-biblioteca.html').then(r => r.text());
    const headerDiv = document.getElementById('header-biblioteca-include');
    ...
    // 3. Cargar footer
    const footHTML = await fetch('./biblioteca/componentes-biblioteca/footer-biblioteca.html').then(r => r.text());
    const footerDiv = document.getElementById('footer-biblioteca-include');
