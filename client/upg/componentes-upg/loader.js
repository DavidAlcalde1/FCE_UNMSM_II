...
async function cargarPlantilla() {
  try {
    // 1. Cargar header
    const headHTML = await fetch('upg/componentes-upg/headerUPG.html').then(r => r.text());
    const headerDiv = document.getElementById('headerUPG-include');
    ...
    // 3. Cargar footer
    const footHTML = await fetch('upg/componentes-upg/footerUPG.html').then(r => r.text());
    const footerDiv = document.getElementById('footerUPG-include');
