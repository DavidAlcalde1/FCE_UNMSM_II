...
async function cargarPlantilla() {
  try {
    // 1. Cargar header
    const headHTML = await fetch('componentes-transparencia/header-transparencia.html').then(r => r.text());
    ...
    // 3. Cargar footer
    const footHTML = await fetch('componentes-transparencia/footer-transparencia.html').then(r => r.text());
