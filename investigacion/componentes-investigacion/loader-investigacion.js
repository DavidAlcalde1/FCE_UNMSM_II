...
async function cargarPlantilla() {
  try {
    // 1. Cargar header
    const headHTML = await fetch('./investigacion/componentes-investigacion/header-investigacion.html').then(r => r.text());
    const headerDiv = document.getElementById('header-investigacion-include');
    ...
    // 3. Cargar footer
    const footHTML = await fetch('./investigacion/componentes-investigacion/footer-investigacion.html').then(r => r.text());
    const footerDiv = document.getElementById('footer-investigacion-include');
